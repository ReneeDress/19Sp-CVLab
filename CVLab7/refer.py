...
starttime = time.time()
path = 'two/uttower/'
#img1 = cv2.imread(path + 'Hanging1.png')  # query
#img2 = cv2.imread(path + 'Hanging2.png')  # train
img1 = cv2.imread(path + 'uttower_left.jpg')  # query
img2 = cv2.imread(path + 'uttower_right.jpg')  # train
...
for i, (m, n) in enumerate(match):
    if (m.distance < 0.75 * n.distance):  # edit origin 0.75
        good.append(m)
print(good, '\n', len(good))
if len(good) > MIN:
    ...
    print("\nsimple stitch cost %f" % (simple - starttime))
    print("\ntotal cost %f" % (final - starttime))
    cv2.imwrite(path + "simplepanorma.png", direct)
    cv2.imwrite(path + "bestpanorma.png", warpImg)
